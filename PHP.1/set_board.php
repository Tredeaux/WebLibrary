...
if ($id) {
    try {
        ...
        if ($result["board"] == 1) {
            try {
                $stmt = $conn->prepare("UPDATE `biofefqs_maindata`.`userdata` SET `board` = 0 WHERE `id` = :a1"); 
                $stmt->execute([":a1" => $id]);
            } catch(PDOException $e) {
            ...
            try {
                $stmt = $conn->prepare("UPDATE `biofefqs_maindata`.`userdata` SET `board` = 1 WHERE `id` = :a1"); 
                $stmt->execute([":a1" => $id]);
            } catch(PDOException $e) {
                //echo "Error: " . $e->getMessage();
            }
