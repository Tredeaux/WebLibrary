...
if ($id) {
    try {
        ...
        if ($result["training"] == 1) {
            try {
                $stmt = $conn->prepare("UPDATE `biofefqs_maindata`.`userdata` SET `training` = 0 WHERE `id` = :a1"); 
                $stmt->execute([":a1" => $id]);
            } catch(PDOException $e) {
                //echo "Error: " . $e->getMessage();
            }
        } else if ($result["training"] == 0) {
            try {
                $stmt = $conn->prepare("UPDATE `biofefqs_maindata`.`userdata` SET `training` = 1 WHERE `id` = :a1"); 
                $stmt->execute([":a1" => $id]);
            } catch(PDOException $e) {
                //echo "Error: " . $e->getMessage();
            }
